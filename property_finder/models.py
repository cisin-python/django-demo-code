...
VALIDATIONS = [
    ('realestate.listing.utils.validation_simple',
...
class Agent(models.Model):
    ...
    phone = models.CharField(
        max_length=15, verbose_name=_('Phone'), null=True, blank=True)
    mobile = models.CharField(
        max_length=15, verbose_name=_('Cellphone'), null=True, blank=True)
    location = models.ForeignKey(
        Location, verbose_name=_('Location'), null=True, blank=True)
    address = models.CharField(
        max_length=200, verbose_name=_('Address'), null=True, blank=True)
    image = ImageField(upload_to='agents/', default='',
                       verbose_name=_('Picture'), null=True, blank=True)
...
class Listing(models.Model):
    ...
    slug = models.SlugField(
        max_length=100, unique=True, blank=False, verbose_name=_('Slug'))
    description = models.TextField(
        verbose_name=_('Description'), null=True, blank=True)
    price = MoneyField(
        default=Money(0, USD), max_digits=12, decimal_places=2, verbose_name=_('Price'))
    location = models.ForeignKey(Location, null=True, blank=True)
    type = models.CharField(_('Listing Type'), max_length=30, choices=TYPES)
    offer = models.CharField(
        max_length=10, choices=OFFERS, verbose_name=_('Offer'))
    ...
    size = models.PositiveIntegerField(
        _('Size(m2)'), default=0, null=True, blank=True)
    coords = models.CharField(max_length=255, default='19.000000,-70.400000', verbose_name=_('Coords'), null=True,
                              blank=True)
    agent = models.ForeignKey(
        Agent, null=True, blank=True, verbose_name=_('Agent'))
    contact = models.ForeignKey(Contact, null=True, blank=True)
    notes = models.TextField(
        max_length=500, verbose_name=_('Private Notes'), null=True, blank=True)
        ...
        for attribute in self.attributelisting_set.all():
            ...
            if attribute.attribute.validation == 'realestate.listing.utils.validation_simple':
                ...
                if attribute.attribute.validation == 'realestate.listing.utils.validation_integer':
                    attributes.append(
                        '%s %s' % (attribute.value, attribute_name))
                else:
                    attributes.append(
                        '%.2f %s' % (attribute.value, attribute_name))
        ...
        if self.has_baths_or_beds:
            ...
            if qs.count() == 0:
                qs = Listing.objects.active(
                    type=self.type, price__range=(lh, rh))
        else:
...
class Attribute(models.Model):
    ...
    validation = models.CharField(
        _('Value type'), choices=VALIDATIONS, max_length=100)
...
class ListingImage(models.Model):
    listing = models.ForeignKey(
        Listing, related_name='images', verbose_name=_('Listing'))
    name = models.CharField(_('Name'), max_length=60)
    image = ImageField(_('Image'), upload_to='listing/')
    added = models.DateTimeField(_('Added'), auto_now_add=True)
    order = models.IntegerField(
        _('Order'), max_length=2, default=99, null=True)
...
class Deal(models.Model):
    ...
    price = MoneyField(
        _('Sale Price'), default=Money(0, USD), max_digits=12, decimal_places=2)
    active = models.BooleanField(_('Active'), default=False)
